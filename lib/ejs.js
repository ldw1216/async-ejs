...
exports.render = function* (template, d, o) {
  ...
  return yield handleCache(opts, template)(data);
};
...
exports.renderFile = function* () {
  ...
  try {
    result = yield handleCache(opts)(data);
  }
  catch(err) {
    return cb(err);
  }
...
Template.prototype = {
  ...
  compile: function () {
    ...
    var returnedFn = function* (data) {
      ...
      return yield fn.apply(opts.context, [data || {}, escape, include, rethrow]);
    };
  ...
  generateSource: function () {
    ...
    if (opts.rmWhitespace) {
      ...
      // work well with `\r`.
      this.templateText =
        this.templateText.replace(/\r/g, '').replace(/^\s+|\s+$/gm, '').replace('<%- include(','<%- yield include(').replace('<%-include(','<%-yield include(');
    }

    // Slurp spaces and tabs before <%_ and after _%>
    this.templateText =
      this.templateText.replace(/[ \t]*<%_/gm, '<%_').replace(/_%>[ \t]*/gm, '_%>').replace('<%- include(','<%-yield include(').replace('<%-include(','<%-yield include(');
